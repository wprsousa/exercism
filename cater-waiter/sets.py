...
def tag_special_ingredients(dish: tuple) -> tuple:
    ...
    """
    dish_list = dish[1]
    special_ingredients = SPECIAL_INGREDIENTS.intersection(dish_list)

    return tuple((dish[0], special_ingredients))
...
def compile_ingredients(dishes: list) -> set:
    ...
    for set_list in dishes:
        new_set.update(set_list)
...
def singleton_ingredients(dishes: list, intersection) -> set:
    ...
    for dish in dishes:
        repeated_ingredients.update(all_ingredients & dish)
        all_ingredients.update(dish)

    singleton_ingredients_set = all_ingredients - repeated_ingredients - intersection
