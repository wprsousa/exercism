...
def make_word_groups(vocab_words) -> str:
    ...
    """
    if not vocab_words and len(vocab_words) < 2:
        return ''

    prefix = vocab_words[0]
    word_with_prefix = [prefix + word for word in vocab_words[1:]]
    result_string = ' :: '.join([prefix] + word_with_prefix)
...
def remove_suffix_ness(word) -> str:
    ...
    last_letter = word_without_suffix[-1]
    if last_letter == 'i':
        changed_word = word_without_suffix[:-1]
        return changed_word + 'y'
